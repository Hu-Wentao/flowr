...
def main() -> int:
    ...
    if mode == "bff-json":
        api_section = (
            ...
            "/// Interactions:\n"
            f"/// - Flow: load-{base.replace('_', '-')}\n"
            "/// - Trigger: startup\n"
            f"/// - Event: [{prefix}Started]\n"
            f"/// - Uses: ui-api [{prefix}BffReq]\n"
            f"/// - Guard: [{prefix}Model].isLoading == false\n"
            f"/// - Pending State: [{prefix}Model].isLoading = true; [{prefix}Model].error = null\n"
            f"/// - Success State: [{prefix}Model].pendingResponseField <- [{prefix}BffRsp].pendingResponseField; [{prefix}Model].isLoading = false\n"
            f"/// - Failure State: [{prefix}Model].error <- error; [{prefix}Model].isLoading = false\n"
