...
from contract_core import ContractError, bracket_refs, class_names, doc_sections, relative_import_uri, require_file
...
def parse_component(component_file: Path) -> ComponentContract:
    ...
    if re.search(r"^\s*(?:import|export|library)\b", contract_source, re.MULTILINE):
        raise ContractError("component contract part must not declare import, export, or library directives")
    ...
    if len(views) != 1:
        raise ContractError("component contract must declare exactly one public XxxView class")
    args = next((name for name in names if name.endswith("PageArgs")), None)
...
def parse_page(page_file: Path) -> PageContract:
    ...
    component_file = page_file.with_name(page_file.name.removesuffix(".page.dart") + ".dart")
    if not relative_import_uri(source, component_file.name):
        raise ContractError(f"page support must import its sibling component library `{component_file.name}`")
    sections = doc_sections(source)
    primary_refs = bracket_refs(sections.get("Component", []))
    if len(primary_refs) != 1:
        raise ContractError("page support must declare exactly one `/// Component: [XxxView]` marker")
    page_classes = [name for name in class_names(source) if name.endswith("Page")]
    if len(page_classes) != 1:
        raise ContractError("page support must declare exactly one public XxxPage class")
