...
def operation_name(service_type: str, request_type: str) -> str:
    ...
    for suffix in ("BffReq", "RequestDto", "Req"):
        ...
        raise ContractError(
            f"request DTO {request_type} must end in BffReq or RequestDto to name a service operation"
        )
...
def uses_request_data_envelope(
    ...
    """Whether this endpoint sends a direct DTO through the configured envelope."""
...
def render_endpoint(
    *,
    service_type: str,
    endpoint: BffEndpoint,
    data_envelope: RequestDataEnvelopeProfile | None,
) -> tuple[str, str | None]:
    ...
    extra_annotation = (
        f"  @Extra(<String, Object>{{{data_envelope.extra_key}: true}})\n"
        if uses_request_data_envelope(endpoint, data_envelope)
        else ""
    )
    if not names:
        method = (
...
{f"import '{data_envelope.extra_import}';" if data_envelope and any(uses_request_data_envelope(endpoint, data_envelope) for endpoint in endpoints) else ''}
...
abstract class {service_type} {{
  factory {service_type}(Dio dio) = _{service_type};
...
def plan_service(
    ...
    *,
    ...
    data_envelope = load_request_data_envelope_profile(Path(component.component_file))
