...
only when runtime wiring is explicitly out of scope.

Final validation for `required` proves a component or declared shared service,
ViewModel injection, an async registered data/command handler, request
construction, awaited service invocation, response-backed state, failure
state, submit/loading recovery, and no navigation before the successful
response.
...
semantic section, provenance, runtime mode, and service ownership together.
...
generated files are complete. `BFF Runtime: required` makes actual service
